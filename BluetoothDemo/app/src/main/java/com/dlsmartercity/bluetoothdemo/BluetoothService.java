...
/**
 * Created by ZhangXiWei on 2018/6/5.
 */
            ...
            if (action.equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)) {
                ...
                switch (state) {
                    case BluetoothDevice.BOND_NONE:
                        ...
                        Toast.makeText(context,"BOND_NONE 删除配对",Toast.LENGTH_SHORT).show();
                        break;
                    case BluetoothDevice.BOND_BONDING:
                        Log.d("aaa", "BOND_BONDING 正在配对");
                        Toast.makeText(context,"BOND_NONE 正在配对",Toast.LENGTH_SHORT).show();
                        break;
                    case BluetoothDevice.BOND_BONDED:
                        Log.d("aaa", "BOND_BONDED 配对成功");
                        Toast.makeText(context,"BOND_NONE 配对成功",Toast.LENGTH_SHORT).show();
                        // 将绑定好的设备添加的已绑定list集合
                        bondDevices.add(unbondDevices.get(itemPosition));
                        // 将绑定好的设备从未绑定list集合中移除
                        unbondDevices.remove(itemPosition);
                        addBondDevicesToListView();
                        addUnbondDevicesToListView();
                        break;
                    default:
